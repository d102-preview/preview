...
def _facial_emotional_recognition(record: Analysis) -> None:
    ...
    logger.info(
        f"Success to encoding and resizing: {datetime(second=(time.time() - st))}"
    )
    ...
    logger.debug(
        f"{len(frame_list)} frames are extracted from the input video: {datetime(second=(time.time() - st))}"
    )
    ...
    logger.debug(
        f"{cnt_face} faces / {len(frame_list)} frames: {datetime(second=(time.time() - st))}"
    )
    ...
    logger.info(f"Process {cnt_face} faces: {datetime(second=(time.time() - st))}")
