...
const Textarea = ({ width = 'w-full', label, borderType = 'all', subText, maxLength, ...props }: ITextareaProps) => {
  ...
  const adjustHeight = () => {
    ...
    textareaRef.current.style.height = 'auto'; // 높이를 auto로 설정하여 초기화
  ...
  return (
    <div className="pb-5">
      ...
      <div
        className={`${width} ${borderType === 'all' ? 'border-2 rounded-lg' : 'border-b-2'} bg-white focus-within:border-MAIN1 overflow-hidden`}
